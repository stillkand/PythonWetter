...
class KalenderRenderHelper:
    def __init__(self, monate, tage):
        self.monatsnamen = monate
        self.tagesnamen = tage

    def render(self, stichtag):
        # Aktuelles Datum
        jahr = stichtag.year
        monat = stichtag.month
        tag = stichtag.day
        ...
        html = f"""
            <table>
                <caption>{self.monatsnamen[monat-1]} {jahr}</caption>
                <tr>{"".join(f"<th>{tag}</th>" for tag in self.tagesnamen)}</tr>
        """
        for woche in tage:
            html += "<tr>"
            for i, tag in enumerate(woche):
                if tag == 0:
                    ...
                    html += self._check_muell_termin(leerungskalender, datum)
                    html += "</div></td>"
            html += "</tr>"
        html += "</table>"
        return html
    
    def render_muell_morgen(self, today):
        returnvalue = ""
        # Aktuelles Datum
        jahr = today.year
    ...
    def _check_muell_termin(self, kalender, suchdatum):
